    ...
    def couchbase_upgrade(self, build, save_upgrade_config=False, forcefully=False):
        ...
        if self.info.type.lower() == 'windows':
            ...
            if save_upgrade_config:
                ...
                command = 'export CB_WAIT_FOR_MASTER_PASSWORD=true; rpm -U /tmp/{0}'.format(build.name)
                if forcefully:
                    command = 'export CB_WAIT_FOR_MASTER_PASSWORD=true; rpm -U --force /tmp/{0}'.format(build.name)
